...
/**
 * testmail.app に送られてくる「［ANA Pay］ご利用のお知らせ」と
 * 「［ANA Pay］チャージ完了のお知らせ」から取引内容を抽出し、
 * それらに対してアクションを起こす関数を管理・通知を飛ばすクラス。
 */
export class ANAPayWatcher {
  ...
  /** List of subscribers to ANA Pay transactions */
  private subscribers: ANAPaySubscriber[];
...
process.stdout.write(dateString+" "+timeString)
      ...
      this.lastPing = now;
      const transactions: ANAPayTransaction[] = [];
  ...
  /**
   * Given text from a ANA Pay email, parse out the transaction details.
   */
  private parseEmailBody(body: string, emaildate: string): ANAPayTransaction {
    const result: ANAPayTransaction = {
      date: "",
      merchant: "",
      totalAmount: 0,
    };
  ...
  private isValidTransaction(transaction: ANAPayTransaction): boolean {
    const {
      date,
      merchant,
      totalAmount,
 ...
 * 読み取れる取引内容。
 */
export interface ANAPayTransaction {
  /** ご利用日時： YYYY/MM/DD */
  date: string;
...
/**
 * 新規のANA Pay取引内容に対してアクションを実行する関数。
 */
export type ANAPaySubscriber = (transactions: ANAPayTransaction[]) => void;
